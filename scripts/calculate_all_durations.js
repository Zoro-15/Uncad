/**
 * scripts/calculate_all_durations.js
...
async function fetchDurationForUid(uid) {
    if (durationCache[uid]) return durationCache[uid];
...
async function main() {
    ...
    const jsonMatch = coursesJs.match(/const COURSES = (\[[\s\S]*?\]);\s*\/\//);
    if (!jsonMatch) {
        console.error("Could not parse COURSES from courses.js");
        return;
    }
    ...
    await mapConcurrent(uidsToFetch, 30, async (lec) => {
...
function findLectureInCourses(uid) {
    for (const c of COURSES) {
        if (c.lectures) {
            ...
            if (l) return { course: c, lecture: l };
        }
    }
    return null;
}

export { COURSES, findCourseById, findCourseByLectureUid, findLectureInCourses };
`;

    fs.writeFileSync(COURSES_JS_PATH, newCoursesJs, 'utf8');
    console.log(`Successfully updated ${updatedCount} lecture durations in src/courses.js!`);
}
